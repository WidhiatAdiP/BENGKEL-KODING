...
<form class="form" method="POST" action="" name="myForm" onsubmit="return(validate());">
    ...
    <select class="form-control" name="id_obat" id="nama_obat" onchange="changeValue(this.value)">
        <option>-Pilih Nama-</option>
        <?php 
        ...
        while ($row = mysqli_fetch_array($result)) {   
          ...
          $jsArray .= "dtObat['" . $row['nama_obat'] . "'] = {
            nama:'".addslashes($row['nama_obat'])."'};\n";   
    }
        ?>
      </select>
    </div>
...
if (isset($_POST['simpan'])) {
    if (isset($_POST['id'])) {
        ...
        $tambah = mysqli_query($koneksi, "INSERT INTO periksa (id_pasien,id_dokter,id_obat,tgl_periksa,catatan) 
                                        VALUES ( 
                                            '" . $_POST['id_pasien'] . "',
                                            '" . $_POST['id_dokter'] . "',
                                            '" . $_POST['id_obat'] . "',
